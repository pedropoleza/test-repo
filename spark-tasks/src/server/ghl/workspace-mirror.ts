...
/**
 * Load the task's current state and push it to the Workspace webhook.
 * No-op unless the location is configured and the webhook env is set.
 */
export async function mirrorTaskToWorkspace(
  ...
  const isDone = !!data.stages.find((s) => s.id === data.t.status)?.isDone;
  const payload: Record<string, unknown> = {
    id: data.t.id,
    title: data.t.title,
    status: isDone ? "done" : "open",
    updatedAt: (data.t.updatedAt ?? new Date()).toISOString(),
  };
  if (data.t.dueDate) payload.dueDate = data.t.dueDate.toISOString().slice(0, 10);
  if (data.assignee) payload.assignee = data.assignee;
  if (data.t.contactId) {
    payload.contactId = data.t.contactId;
    payload.url = ghlContactUrl(locationId, data.t.contactId);
  }
  ...
  for (let i = 0; i < MAX_ATTEMPTS; i++) {
    try {
      ...
      if (res.ok) return;
      // 4xx (bad signature / payload) won't be fixed by retrying.
      if (res.status >= 400 && res.status < 500) {
        console.error(
          `[workspace-mirror] ${res.status} for task ${taskId}: ${(await res.text()).slice(0, 120)}`,
        );
        return;
      }
      ...
      console.warn(`[workspace-mirror] attempt ${i + 1} failed (${taskId}): ${msg}`);
      if (i < MAX_ATTEMPTS - 1) await new Promise((r) => setTimeout(r, BACKOFF_MS[i]));
    }
  }
  console.error(`[workspace-mirror] gave up mirroring task ${taskId}`);
}
