...
const TIERS_LIST = ["starter", "growth", "scale"];

/**
 * Cria (idempotente) os 3 promotion codes por-plano de uma location,
 * apontando pros cupons compartilhados (INDICACAO_*). Cada code:
 *   ${base}OFF${TIER}  →  coupon do tier (desconto + applies_to product)
 *   metadata: { indicador_location, tier, role: 'indicado_perplan' }
 *
 * O desconto + restrição de produto vêm do coupon compartilhado; o
 * promo code só carrega QUEM indicou. Plan-restricted automaticamente.
 *
 * planCouponIds = { starter: coupId, growth: coupId, scale: coupId }
 * (vem de plan_config.coupon_id).
 *
 * Retorna { starter: code, growth: code, scale: code } (os que deram certo).
 */
export async function ensureLocationPlanCoupons(locationId, locationName, planCouponIds) {
  const stripe = stripeClient();
  const base =
    deriveCouponBase(locationName) ||
    `LOC${(locationId || "").slice(0, 6).toUpperCase()}`;
  ...
  for (const tier of TIERS_LIST) {
    const couponId = planCouponIds?.[tier];
    if (!couponId) continue;
    const tierUp = tier.toUpperCase();

    // Já existe um promo code dessa location pra esse tier? (idempotência)
    // Formato canônico: ${BASE}${TIER}OFF  → ex: SPARKSTARTEROFF
    let code = `${base}${tierUp}OFF`;
    let created = null;
    for (let i = 0; i < 5 && !created; i++) {
      const suf = i === 0
        ? ""
        : `${(locationId || "").slice(0, 4).toUpperCase().replace(/[^A-Z0-9]/g, "")}${i === 1 ? "" : i}`;
      const attempt = `${base}${tierUp}${suf}OFF`;
      try {
        created = await stripe.promotionCodes.create({
          coupon: couponId,
          code: attempt,
          metadata: {
            indicador_location: locationId,
            tier,
            role: "indicado_perplan",
            source: "spark-referral-hub",
          },
        });
        out[tier] = created.code;
      } catch (err) {
        if (err?.code === "resource_already_exists" || /already/i.test(err?.message || "")) {
          // code colidiu — tenta achar o existente que seja DESTA location
          try {
            const found = await stripe.promotionCodes.list({ code: attempt, limit: 1 });
            const pc = found.data?.[0];
            if (pc && pc.metadata?.indicador_location === locationId) {
              out[tier] = pc.code;
              created = pc;
              break;
            }
          } catch {}
          continue; // tenta próximo sufixo
        }
        // erro real — pula esse tier
        break;
      }
    }
  }
